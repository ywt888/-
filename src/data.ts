...
export const PROJECTS: Project[] = [
  {
    id: "synapse-canvas",
    category: "人工智能应用 & 生产力工具",
    name: "Synapse Canvas (神经画布)",
    title: "重塑 AI 时代的专业级工作流",
    subtitle: "让大语言模型不再局限于单向对话框，而是无限网格上的直觉空间操作",
    keywords: ["空间隐喻", "AI UX", "节点分支", "极简卡片", "人机交互"],
    year: "2026",
    role: "创始人 & 首席产品设计师",
    coreStatement: "探索超越传统单维对话气泡的 AI 直觉式空间界面。通过卡片物理位置、语义连线、平行分支（Forking）重新定义知识沉淀与复杂推理。",
    timeline: "2025.10 - 2026.04",
    projectType: "创始人兼个人独立项目",
    background: {
      industryProblem: "绝大多数大语言模型应用仍局限于单维度的对话气泡流（Chat UI），这在需要多任务协作、信息回溯、平行版本比较的专业创造力场景中极具局限性。信息容易沉没，缺乏可视化的结构。",
      userProblem: "用户在同时探索多条生成路径时（例如对比不同 prompt 输出或进行复杂的逻辑推演），难以组织前后文脉络，不断在庞大文本中往复滚动，认知负荷高达极值，完全缺乏对人机协作过程的掌控感。",
      businessGoal: "设计下一代 AI 专业化工作空间（AI Workspace Pro），将用户在复杂脑力任务下的信息整理效率提升 240%，降低多任务切换流失率，为 AI 辅助创意与研究软件设立全新体验标准。"
    },
    strategy: {
      insights: [
        "人类大脑是高度空间组织型的：我们将想法放在不同的位置，天然就带有逻辑、权重和进度的物理暗示。",
        "AI 的输出不应该作为终点，而应该作为可以被拆解、拖拽、修改并流入下一个分支的「交互节点」。",
        "在创作过程中，多路径并行尝试（Forking）是捕获大模型随机性、淘洗高质量内容的最高频操作。"
      ],
      uxStrategy: "舍弃单链式对话，采用二级无限网格。引入「神经元节点」概念，支持随处双击生成独立 prompt 卡片、手势连线作为上下文关联线、单卡任意分支对撞。引入物理级碰撞吸附机制，让相似层级的节点自动聚群，从而保持脑暴时的整洁度。",
      designDirection: "极致冷峻的单色美学。极细线条、定制等宽字体混排、极高文字对比度，辅以优雅的微阴影和毫秒级瞬时微动。去除一切非必要卡片边框遮挡，让文字成为画布的首要材质。"
    },
    showcaseSections: [
      {
        title: "研究阶段 (Research)",
  ...
  },
  {
    id: "krypton-debugger",
    category: "智能开发工具 & 调试层",
    name: "Krypton Core (智能编译器 UX)",
    title: "打破人与代码的抽象阻限",
    subtitle: "让极其复杂的分布式服务调用与宏编译过程，变为直观可视、即点即编的代码拓扑图谱",
    keywords: ["编译器交互", "三维依赖追踪", "等宽排版", "状态漫游", "双向链条"],
    year: "2025",
    role: "联创 & 设计总监",
    coreStatement: "将繁重晦涩的程序编译与错误堆栈转换成完全交互式的流线面包板模型，帮助高并发软件开发者在极短时间内发现隐蔽的数据竞争与时序漏洞。",
    timeline: "2024.11 - 2025.08",
    projectType: "商业联合创始人项目",
    background: {
      industryProblem: "多核并发、分布式调用和庞大项目的宏编译状态是一张无法直接眼见、难以完全理解的重度迷网。控制台疯狂刷屏的纯文本报错和堆栈信息，割裂了逻辑和空间关系。",
      userProblem: "开发者需要不断在本地编辑器、监控面板以及命令行三者之间跳脱来拼凑报错的全貌。调试一个锁态或竞态通常需要数个小时的重复推理。",
      businessGoal: "为企业和高精尖极客开发者推出新一代 Krypton 编译级界面，通过精细设计语言打造全球最顶级的并发逻辑观测仪表盘，获取极高的开发者溢价。"
    },
    strategy: {
      insights: [
        "报错并不是一个简单的「故障发生点」，它是某个庞大异步逻辑序列执行到崩溃的「最后快照」。",
        "代码的组织并不是扁平的文件目录，而是以导入导出连线构筑的、不断收敛的多向非循环图 (DAG)。",
        "开发者真正的舒畅感，来自于报错信息与具体发生文件及修改入口，能无缝展现在同一个视觉层级内。"
      ],
